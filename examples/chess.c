...
ch_mcts_tree* ch_free_mcts(ch_mcts_tree* root) {
    ...
    if (root->total_wins) { FREE(root->total_wins); root->total_wins = NULL; }
...
float ch_network_predict(network *net, float *input, int player)
{
    ...
    net->train = 0;
    forward_network(net);
    return player == 0 ? net->output[(0)] - net->output[(1)] : net->output[(1)] - net->output[(0)];
}
...
int ch_end_move(char* sessionId, char *fen, char* move, int idx)
{
    if (fen[0] == '\0' || move[0] == '\0') return 1;
    int is_draw_in_c = ch_is_three_fold_repetition(sessionId, fen);
    return ch_is_end(fen, idx) || is_draw_in_c;
}

int ch_mate_move(char *ko, int idx)
{
    return ch_is_checkmate_move(ko, idx);
}
...
float *ch_move(float *board, int indext) {
    char* pfen = ch_board_to_fen(board);
    int mcounter = 0;
    char* mfen = ch_do_legal(pfen, indext);
    if (mfen == NULL) {
...
char *ch_move_fen(char* fen, int indext) {
    char* mfen = ch_do_legal(fen, indext);
    if (mfen == NULL) {
        fprintf(stderr, "try (%i) on %s\n", indext, fen);
    }
    return mfen;
...
void ch_self_study_train_self_step(char*sessionId, char *sfen, char *valid_fen, char *valid_move, network *net, int level, int idx, float pow, float value) {
    ...
    if (player == 0) { y[0] = value; y[1] = pow; }
    if (player == 1) { y[1] = value; y[0] = pow; }
...
float* ch_moves_similarity_ai(network* net, char** moves, int n, float *prev, char* sfen) {
    ...
    for (int idx = 0; idx < n; ++idx) {
        ...
        similarities[idx] = ch_network_predict(net, x, player);
        FREE(x);
        FREE(next);
    }
    FREE(fen);
...
ch_mcts_tree *ch_expand_mcts(ch_mcts_tree *parent, float *board, network *net, char* sfen, int level)
{
    ...
    if (ch_get_all_valid_moves(fen, &valid_fen, &valid_moves, &valid_moves_count)) {
        ...
        root->total_wins = (float *) CALLOC(root->count, sizeof(float));
        ...
        float *similar = ch_moves_similarity(net, valid_moves, root->count, root->board, sfen);
        for (int i = 0; i < root->count; ++i) { root->ucb[i] = 0; root->value[i] = similar[i]; root->visit_count[i] = 1; }
        FREE(similar);
...
ch_mcts_tree* ch_expansion_mcts(ch_mcts_tree* leaf, int index, network *net, char* sfen, int level) {
    ch_mcts_tree* current = leaf;
    if (current->children[index] == NULL) {
        float *next = ch_move(current->board, index);
        ch_mcts_tree* expanded = ch_expand_mcts(current, next, net, sfen, level);
        if (expanded != NULL) {
            current = current->children[index] = expanded;
        }
        FREE(next);
    } else {
...
int ch_select_mcts(ch_mcts_tree* leaf, ch_mcts_tree* root, network* net, float cpuct, char* sfen, int level) {
    ...
    for (int i = 0; i < current->count; i++) {
        float w = (float)current->total_wins[i];
        float n = (float)current->visit_count[i] + 1e-6f;
        float N = (float)root->total_count;

        float uct = (w / n) + cpuct * sqrtf(logf(N + 1) / n);
        current->ucb[i] = current->value[i] + uct;

        if (current->ucb[i] > best_ucb) {
            best_ucb = current->ucb[i];
            best_child = i;
        }
    }
...
ch_mcts_tree* ch_backpropagate_mcts(ch_mcts_tree* leaf, float result) {
    ...
    while (current != NULL) {
        ...
        if (best_index >= 0 && best_index < current->count) {
            current->total_wins[best_index] -= result;
        }
        result = -result;
...
float ch_simulate_heuristic_ai(ch_mcts_tree* leaf, ch_mcts_tree* root, network* net, char* sfen, int player) {
    ch_mcts_tree* current = leaf;
    if (current) {
        int index = ch_index(current->player, current->visit_count, current->count);
        float *prev = current->board;
        float *next = ch_move(prev, index);
        float *x = (float *) CALLOC((0)+(2)*(BOARD_SIZE), sizeof(float));
        for (int i = 0; i < (1)*(BOARD_SIZE); ++i) {
            x[2 * i] = prev[i];
            x[2 * i + 1] = next[i];
        }
        current->value[index] = ch_network_predict(net, x, player);
        FREE(x);
        FREE(next);
...
float ch_simulate_heuristic(ch_mcts_tree* leaf, ch_mcts_tree* root, network* net, char* sfen, int player) {
    ch_mcts_tree* current = leaf;
    if (current) {
        int index = ch_index(current->player, current->visit_count, current->count);
        float *board = current->board;
        float *next = ch_move(board, index);
        float powW = 0;
        float powB = 0;
        ch_eval_the_board(sfen, next, &powW, &powB);
        current->value[index] = player == 0 ? powW : powB;
        FREE(next);
        return current->value[index];
    }
    return 0.f;
...
ch_mcts_tree* ch_search_mcts(ch_mcts_tree* leaf, ch_mcts_tree* root, network* net, float cpuct, char* sfen, int level, int player) {
    ...
    leaf = ch_expansion_mcts(leaf, index, net, sfen, level);
    float result = ch_simulate_heuristic(leaf, root, net, sfen, player);
    if (leaf->parents >= 4) {
...
int get_best_move_index(ch_mcts_tree* root) {
    int best_move = -1;
    int max_visits = -1;
    float max_wins = -FLT_MAX;

    for (int i = 0; i < root->count; i++) {
        float wins = root->total_wins[i];
...
void ch_run_mcts(char* sessionId, network *net, char* valid_fen, char** valid_moves,  float *board, int n, float cpuct, float secs, char* sfen, int level)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (q->index != -1) {
            ...
            if (idx != q->index && q->index >= 0 && q->index < root->count && pow < q->value) {
                idx = q->index;
                pow = q->value;
    ...
    if (0) {
        ...
        ch_print_matrix(root, "total_wins", root->total_wins);
        fprintf(stderr, "moves: ");
        for (int i = 0; i < root->count; ++i) {
            fprintf(stderr, "%i:[%s] ", i, valid_moves[i]);
        }
        fprintf(stderr, "\n");
    }
...
int ch_pick_move_mcts(char* sessionId, char* sfen, char* valid_fen, char** valid_moves, ch_moves ch_m, network *net, int n, int level, int *solver, float *pow, float *value)
{
    ...
    if (level <= 0 || trivial_player == player) {
        *solver = 2; // ML
        float *pows = NULL;
        int idx = ch_eval_best_trivial_move(sfen, valid_fen, 4, pow, &pows, &counter);
        *value = pows[idx];
...
#ifdef CH_ENGINE
    ch_run_mcts(sessionId, net, valid_fen, valid_moves, board, (level+1) * 20000000, 1.4f, (float) (level+1) * 0.2f, sfen, level);
#else
    ch_run_mcts(sessionId, net, valid_fen, valid_moves, board, (level+1) * 20000000, 1.4f, (float) (level+1) * 0.2f, sfen, level);
#endif
    ...
    *value = q->value;
    float *next = ch_fen_to_board(valid_fen);
    float powW = 0;
    float powB = 0;
    ch_eval_the_board(sfen, next, &powW, &powB);
    q->value = player == 0 ? powW : powB;
    *pow = q->value;
    q->index = idx;

    fprintf(stderr, "pick (%s): %ld(%s): count: (%i) checked: (%i) power: (%.7f) index: (%i) move: (%s) value: (%.8g)\n", *solver == 1 ? "ai" : "ml", net->nsteps, player == 0 ? "w" : "b", n, counter, *pow, idx, valid_moves[idx], *value);

    float *moved = ch_move(next, idx);
...
int ch_process_file(char *file_name) {
    ...
    if (ch_fopen(ffiname, &sessionId, &fen, &fen_move, &level, &sfen)) {
        ...
        if (fen_move != NULL && strcmp(fen_move, "") != 0) {
            ...
            int fen_next_idx = 0;
            int fen_next_cnt = 0;
            if (ch_board_after_move(fen, fen_move, &fen_next, &fen_next_idx, &fen_next_cnt)) {
                if (fen_next == NULL) {
        ...
        if (fen != NULL && fen[0] != '\0' && ch_get_all_valid_moves(fen, &valid_fen, &valid_moves, &valid_moves_count)) {
            ...
            if (indext != -1 && ch_end_move(sessionId, fen, valid_moves[indext], indext)) {
            ...
            if (ch_get_all_valid_moves_after(fen, fen_move, &valid_fen, &valid_moves, &valid_moves_count)) {
                ...
                if (valid_moves[indext] != NULL && strcmp(valid_moves[indext], "") != 0) {
                    ...
                    int fen_next_idx = 0;
                    int fen_next_cnt = 0;
                    if (ch_board_after_move(valid_fen, valid_moves[indext], &fen_next, &fen_next_idx,&fen_next_cnt)) {
                        FREE(fen_next);
                    }
                }
...
ch_board_state ch_self_learn_step(char* sessionId, char* sfen, int level, network *net, char *fen, char *fen_move, int learn) {
    ...
    if (ch_get_all_valid_moves(fen, &valid_fen, &valid_moves, &valid_moves_count)) {
        ...
        if (indext == -1) {
            ...
            ch_put_into_game_queue(sessionId, valid_fen, valid_moves[indext], indext, net, sfen);
            if (learn == 1) ch_self_study_train_self_step(sessionId, sfen, valid_fen, valid_moves[indext], net, level, indext, pow, value);
            strcpy(return_value.fen, valid_fen);
            strcpy(return_value.move, valid_moves[indext]);
            return_value.indext = indext;
            for (int j = 0; j < valid_moves_count; ++j) FREE(valid_moves[j]);
...
void test_tchess(int argc, char **argv, char *cfgfile, char *weight_file) {
    ...
    while (net->nsteps < net->max_batches) {
        ...
        if (ch_end_move(sessionId, valid_fen, valid_fen_move, move_state.indext)) {
        ...
        float* board = ch_fen_to_board(valid_fen);
        float* next = ch_move(board, move_state.indext);
...
void ch_train_possible_checkmate(char *sessionId, ch_board_state move_state, char *sfen, network *net, int level, int deep) {
    if (deep > 0) {
        ...
        if (ch_get_all_valid_moves(move_state.fen, &valid_maybe_fen, &valid_maybe_moves, &valid_maybe_moves_count) && valid_maybe_moves_count > 0) {
            for (int i = 0; i < valid_maybe_moves_count; ++i) {
                if (ch_mate_move(valid_maybe_fen, i)) {
...
void test_echess(int argc, char** argv, char *cfgfile, char *weight_file) {
    ...
    while ((nread = getline(&buff, &len, stdin)) > 0) {
        ...
        if (strncmp(buff, "ucinewgame", 10) == 0) {
            ...
            int count = 0;
            char* fen = ch_analyze_pos(buff, &sfenn, &mfenn, &moves, &move, &count);
            ...
            if (exists) {
                ...
                if (ch_board_after_move(valid_fen_last, moves[count - 1], &mfen_next, &mfen_next_idx, &mfen_next_cnt)) {
                    ...
                    ch_self_study_train_self_step(sessionId, sfen, valid_fen_last, moves[count - 1], net, level, mfen_next_idx, pow0, 1.f);
                ...
                if (ch_board_after_move(valid_fen_next, moves[addon - 1], &mfen_next, &mfen_next_idx, &mfen_next_cnt)) {
                    ...
                    ch_self_study_train_self_step(sessionId, sfen, valid_fen_next, moves[addon - 1], net, level, mfen_next_idx, pow1, 1.f);
                    ...
                    for (int i = addon - 1; i < count; ++i) {
                        ...
                        if (ch_board_after_move(valid_fen_next, moves[addon - 1], &mfen_next, &mfen_next_idx, &mfen_next_cnt)) {
                            ...
                            ch_self_study_train_self_step(sessionId, sfen, valid_fen_next, moves[addon - 1], net, level,mfen_next_idx, pow2, 1.f);
