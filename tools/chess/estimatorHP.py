...
def safe_readline(pipe, timeout=5):
...
EVALUATE_PATH = "/Applications/HIARCS Chess Explorer Pro.app/Contents/MacOS/DeepHiarcs15.4-engine"
INITIAL_ELO = 800
EVALUATE_ELO = min(max(750, 1600), 3000)
K_FACTOR = 20
GAMES_TO_PLAY = 50
MOVE_TIME_MS = 2000
...
CSV_FILE = "./estimatorH.csv"

if not os.path.isfile(CSV_FILE):
    with open(CSV_FILE, mode="w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Game", "Result", "MyELO", "OpponentELO"])
...
line, = ax.plot(elo_history, color='lime', label="Estimated Elo")
ax.set_xlim(0, GAMES_TO_PLAY)
ax.set_ylim(750, 3000)
ax.set_title("Dynamic Elo Estimation", color='white')
ax.set_xlabel("Games", color='white')
...
evaluate = start_engine(EVALUATE_PATH)
send_cmd(evaluate, f"setoption name UCI_LimitStrength value true")
send_cmd(evaluate, f"setoption name UCI_Elo value {EVALUATE_ELO}")
...
for game_num in range(1, GAMES_TO_PLAY + 1):
    board = chess.Board()
    if game_num % 2 == 1:
        engines = [my_engine, evaluate]
        names = [my_engine_name, "HIARCS"]
    else:
        engines = [evaluate, my_engine]
        names = ["HIARCS", my_engine_name]
    ...
    with open(CSV_FILE, mode="a", newline="") as file:
        writer = csv.writer(file)
        writer.writerow([game_num, result_str, current_elo, EVALUATE_ELO])
    ...
    exp = expected_score(current_elo, EVALUATE_ELO)
    current_elo = update_elo(current_elo, exp, score)
    elo_history.append(current_elo)

    line.set_ydata(elo_history)
    line.set_xdata(range(len(elo_history)))
    ax.relim()
    ax.autoscale_view()
    plt.draw()
    plt.pause(0.01)
