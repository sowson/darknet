...
layer make_gaussian_yolo4_layer(int batch, int w, int h, int n, int total, int *mask, int classes, int max_boxes)
{
    ...
    l.forward = forward_gaussian_yolo4_layer;
    l.backward = backward_gaussian_yolo4_layer;
#ifdef GPU
    l.forward_gpu = forward_gaussian_yolo4_layer_gpu;
    l.backward_gpu = backward_gaussian_yolo4_layer_gpu;
...
box get_gaussian_yolo4_box(float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride, YOLO4_POINT yolo4_point)
{
    box b;
    ...
    if (yolo4_point == YOLO4_CENTER) {
    }
    else if (yolo4_point == YOLO4_LEFT_TOP) {
        b.x = (i + x[index + 0 * stride]) / lw + b.w / 2;
        b.y = (j + x[index + 2 * stride]) / lh + b.h / 2;
    }
    else if (yolo4_point == YOLO4_RIGHT_BOTTOM) {
        b.x = (i + x[index + 0 * stride]) / lw - b.w / 2;
        b.y = (j + x[index + 2 * stride]) / lh - b.h / 2;
    }
...
float delta_gaussian_yolo4_box(box truth, float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, float *delta,
    float scale, int stride, float iou_normalizer, IOU_LOSS iou_loss, float uc_normalizer, int accumulate, YOLO4_POINT yolo4_point, float max_delta)
{
    box pred = get_gaussian_yolo4_box(x, biases, n, index, i, j, lw, lh, w, h, stride, yolo4_point);
...
void averages_gaussian_yolo4_deltas(int class_index, int box_index, int stride, int classes, float *delta)
{
...
void delta_gaussian_yolo4_class(float *output, float *delta, int index, int class_id, int classes, int stride, float *avg_cat, float label_smooth_eps, float *classes_multipliers, float cls_normalizer)
{
    int n;
    if (delta[index]){
        float y_true = 1;
...
int compare_gaussian_yolo4_class(float *output, int classes, int class_index, int stride, float objectness, int class_id, float conf_thresh)
{
    int j;
    for (j = 0; j < classes; ++j) {
    ...
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    box pred = get_gaussian_yolo4_box(l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, net.w, net.h, l.w*l.h, l.yolo4_point);
                    ...
                    for(t = 0; t < l.max_boxes; ++t){
                        ...
                        int class_id_match = compare_gaussian_yolo4_class(l.output, l.classes, class_index, l.w*l.h, objectness, class_id, 0.25f);
                    ...
                    if (best_match_iou > l.ignore_thresh) {
                        ...
                        if (l.objectness_smooth) {
                            ...
                            int class_id = net.truth[best_match_t*(4 + 1) + b*l.truths + 4];
                            if (l.map) class_id = l.map[class_id];
                            delta_gaussian_yolo4_class(l.output, l.delta, class_index, class_id, l.classes, l.w*l.h, 0, l.label_smooth_eps, l.classes_multipliers, l.cls_normalizer);
                        }
                    ...
                    if (best_iou > l.truth_thresh) {
                        ...
                        delta_gaussian_yolo4_box(truth, l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, net.w, net.h, l.delta, (2-truth.w*truth.h), l.w*l.h, l.iou_normalizer * class_multiplier, l.iou_loss, l.uc_normalizer, 1, l.yolo4_point, l.max_delta);
                    }
                }
            }
        }
        for(t = 0; t < l.max_boxes; ++t){
            ...
            if(mask_n >= 0){
                ...
                const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
                float iou = delta_gaussian_yolo4_box(truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h, net.w, net.h, l.delta, (2-truth.w*truth.h), l.w*l.h, l.iou_normalizer * class_multiplier, l.iou_loss, l.uc_normalizer, 1, l.yolo4_point, l.max_delta);
                ...
                delta_gaussian_yolo4_class(l.output, l.delta, class_index, class_id, l.classes, l.w*l.h, &avg_cat, l.label_smooth_eps, l.classes_multipliers, l.cls_normalizer);
            ...
            for (n = 0; n < l.total; ++n) {
                ...
                if (mask_n >= 0 && n != best_n && l.iou_thresh < 1.0f) {
                    ...
                    if (iou > l.iou_thresh) {
                        ...
                        int box_index = entry_gaussian_index(l, b, mask_n*l.w*l.h + j*l.w + i, 0);
                        const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
                        float iou = delta_gaussian_yolo4_box(truth, l.output, l.biases, n, box_index, i, j, l.w, l.h, net.w, net.h, l.delta, (2 - truth.w*truth.h), l.w*l.h, l.iou_normalizer * class_multiplier, l.iou_loss, l.uc_normalizer, 1, l.yolo4_point, l.max_delta);
        ...
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    averages_gaussian_yolo4_deltas(class_index, box_index, stride, l.classes, l.delta);
                }
            }
        }
    }
...
int get_gaussian_yolo4_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, int relative, detection *dets, int letter)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            ...
            if (objectness > thresh) {
                ...
                dets[count].bbox = get_gaussian_yolo4_box(predictions, l.biases, l.mask[n], box_index, col, row, l.w, l.h, netw, neth, l.w*l.h, l.yolo4_point);
                ...
                dets[count].points = l.yolo4_point;
                //if (l.yolo4_point != YOLO4_CENTER) dets[count].objectness = objectness = 0;
    ...
    correct_gaussian_yolo4_boxes(dets, count, w, h, netw, neth, relative, letter);
    return count;
}
...
void forward_gaussian_yolo4_layer_gpu(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b)
    {
        for(n = 0; n < l.n; ++n)
        {
            ...
            scal_add_offset_gpu(l.w*l.h, l.scale_x_y, -0.5*(l.scale_x_y - 1), l.output_gpu, index, 1);      // scale x
            // y : mu, sigma
            index = entry_gaussian_index(l, b, n*l.w*l.h, 2);
            activate_array_offset_gpu(l.output_gpu, index, 2*l.w*l.h, LOGISTIC);
            scal_add_offset_gpu(l.w*l.h, l.scale_x_y, -0.5*(l.scale_x_y - 1), l.output_gpu, index, 1);      // scale y
            // w : sigma
            index = entry_gaussian_index(l, b, n*l.w*l.h, 5);
            activate_array_offset_gpu(l.output_gpu, index, l.w*l.h, LOGISTIC);
            // h : sigma
            index = entry_gaussian_index(l, b, n*l.w*l.h, 7);
            activate_array_offset_gpu(l.output_gpu, index, l.w*l.h, LOGISTIC);
            // objectness & class
    ...
    if (net.truth) {
        ...
        opencl_pull_array(net.truth_gpu, truth_cpu, num_truth);
    }
    network cpu_state = net;
    cpu_state.index = net.index;
    cpu_state.train = net.train;
    cpu_state.truth = truth_cpu;
    cpu_state.input = in_cpu;
    forward_gaussian_yolo4_layer(l, cpu_state);
...
void backward_gaussian_yolo4_layer_gpu(const layer l, network net)
{
    axpy_gpu(l.batch*l.inputs, l.delta_normalizer, l.delta_gpu, 1, net.delta_gpu, 1);
}
#endif
