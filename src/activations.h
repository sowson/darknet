...
static inline float relu10_activate(float x){return x < 0 ? 0 : x > 10 ? 10 : x;}
static inline float lelu_activate(float x){return 8.f * tanhf(x);}
...
static inline float relu10_gradient(float x){return x > 0 || x < 10 ? 1 : 0;}
static inline float lelu_gradient(float x){float c = coshf(x); return 8.f / (powf(c, 2.f));}
